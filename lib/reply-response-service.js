...
const { categorizeReply: categorizeReplyText } = require('./reply-categorization-service');
...
class ReplyResponseService {
  ...
  /**
   * Categorize a reply email.
   * Uses AI-primary path (with OOO fast-path and keyword fallback).
   */
  async categorizeReply(replyId, userId) {
    const reply = await this._getReply(replyId, userId);
    if (!reply) throw new Error('Reply not found');

    // Get prospect context for richer AI classification
    let prospectName, companyName;
    if (reply.prospect_id) {
      try {
        const pResult = await this.pool.query(
          'SELECT company_name, contact_first_name, contact_last_name FROM prospects WHERE id = $1',
          [reply.prospect_id]
        );
        if (pResult.rows[0]) {
          companyName = pResult.rows[0].company_name;
          prospectName = [pResult.rows[0].contact_first_name, pResult.rows[0].contact_last_name]
            .filter(Boolean).join(' ') || null;
        }
      } catch { /* Non-blocking */ }
    }

    const categorization = await categorizeReplyText(
      reply.subject,
      reply.body_text || reply.body_html || '',
      { ai: this.ai, userId, prospectName, companyName }
    );

    // Map new categorization fields to existing schema
    const dbCategory = categorization.category;
    const aiCategorization = {
      category: categorization.category,
      confidence: categorization.confidence,
      reasoning: categorization.summary,
      ooo_return_date: categorization.return_date,
      key_objection: categorization.key_concern,
    ...
    await this.pool.query(
      ...
      [dbCategory, JSON.stringify(aiCategorization), replyId]
    );
