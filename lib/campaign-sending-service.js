...
const ColdEmailSender = require('./cold-email-sender');
const DeliverabilityService = require('./deliverability-service');

/**
 * Campaign Sending Service
 * Manages scheduled sends, multi-step sequences, and stop-on-reply logic.
 * Uses ColdEmailSender for outbound campaign emails (SES/Mailgun).
 * Postmark (via EmailService) is reserved for transactional email only.
 *
 * Deliverability gates (added Tier 1):
 *   - Suppression list check before each send
 *   - Warmup plan daily capacity enforcement
 *   - Post-send outcome recording
 */
class CampaignSendingService {
  constructor(pool) {
    this.pool = pool;
    this.emailService = new EmailService(pool);
    this.coldSender = new ColdEmailSender();
    this.deliverability = new DeliverabilityService(pool);
    this.logger = console;
  }
  ...
  async processSendingQueue() {
    try {
      // Get pending emails with row-level locking to prevent double-sends
      const result = await this.pool.query(
        `SELECT
          q.id as queue_id,
          q.campaign_id,
      ...
      for (const item of queueItems) {
        ...
        // Deliverability gate 1: suppression list check
        const suppressed = await this.deliverability.isSuppressed(item.recipient_email);
        if (suppressed) {
          await this.pool.query(
            `UPDATE campaign_sending_queue SET status = 'suppressed', updated_at = NOW() WHERE id = $1`,
            [item.queue_id]
          );
          this.logger.info(`[Deliverability] Suppressed send to ${item.recipient_email}`);
          continue;
        }

        // Deliverability gate 2: warmup daily capacity
        const remaining = await this.deliverability.getRemainingDailyCapacity(item.campaign_id);
        if (remaining === 0) {
          this.logger.info(`[Deliverability] Campaign ${item.campaign_id} warmup daily cap reached`);
          break; // All further items for this campaign will also be blocked today
        }
        ...
        // Send email via cold ESP (not Postmark)
        const sendResult = await this.coldSender.send({
          to: item.recipient_email,
          toName: item.recipient_name,
          from: item.sender_email,
          fromName: item.sender_name,
          replyTo: item.reply_to_email || item.sender_email,
          subject: item.subject_line,
          html: item.email_body,
          metadata: {
        ...
        if (sendResult.success) {
          ...
          // Deliverability: record outcome + warmup counter
          await this.deliverability.recordOutcome(item.generated_email_id, item.recipient_email, sendResult);
          await this.deliverability.recordWarmupSend(item.campaign_id);
