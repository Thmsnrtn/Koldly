...
const { categorizeReply } = require('./reply-categorization-service');

function registerInboxRoutes(app, pool) {
  ...
  app.get('/api/inbox/replies', requireAuth, [
    ...
    try {
      ...
      // Get total count
      let countQuery = `SELECT COUNT(*) as total FROM prospect_reply_inbox WHERE 1=1`;
      const countParams = [];
  ...
  app.get('/api/inbox/replies/:id', requireAuth, [
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT * FROM prospect_reply_inbox WHERE id = $1`,
        [id]
      );
  ...
  app.post('/api/inbox/replies/:id/categorize', requireAuth, [
    ...
    try {
      ...
      // Re-categorize
      const { category, confidence } = categorizeReply(reply.reply_subject, reply.reply_body);

      // Update the reply
      const updateResult = await pool.query(
        `UPDATE prospect_reply_inbox
         SET reply_category = $1, category_confidence = $2, updated_at = NOW()
         WHERE id = $3
         RETURNING *`,
