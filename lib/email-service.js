 ...
 *
 * IMPORTANT: Uses Postmark, which is a TRANSACTIONAL email provider.
 * Postmark's Acceptable Use Policy prohibits unsolicited bulk/cold email.
 * For production cold outreach, switch to a cold-email-friendly provider
 * (e.g., Amazon SES, Mailgun, SendGrid with proper warm-up).
 * See: https://postmarkapp.com/support/article/800-can-i-send-bulk-email
 */
...
class EmailService {
  ...
  async validateEmail(email) {
    ...
    let hasKnownMx = false;
    try {
      const dns = require('dns').promises;
      const mxRecords = await dns.resolveMx(domain);
      hasKnownMx = mxRecords && mxRecords.length > 0;
    } catch (dnsErr) {
      // DNS lookup failed — domain may not exist or have no MX records
      // Fall back to format-only validation (don't block on DNS issues)
      this.logger.warn(`[Email] MX lookup failed for ${domain}: ${dnsErr.code || dnsErr.message}`);
      hasKnownMx = false;
    }
...
<p style="font-size: 12px; color: #999; margin-top: 20px;">
   ...
   * Bypasses spam check and rate limits since these are system emails.
   */
  async sendTransactionalEmail(to, subject, html, text = null) {
    if (!this.apiKey) {
      this.logger.info(`[MOCK] Transactional email to ${to}: ${subject}`);
      return { success: true, message_id: `mock-txn-${Date.now()}` };
    }

    try {
      const payload = {
        From: process.env.TRANSACTIONAL_FROM_EMAIL || process.env.FROM_EMAIL || 'noreply@koldly.com',
        To: to,
        Subject: subject,
        HtmlBody: html,
      ...
      const response = await this.makePostmarkRequest('POST', '/email', payload);
      return { success: true, message_id: response.MessageID };
    } catch (err) {
      this.logger.error('Transactional email error:', err);
